...
vendors_df = spark.read.format("jdbc") \
        .option("url", f"jdbc:sqlite:{DB_PATH}") \
...
print(f"Vendors: {vendors_df.count():,} records")
...
# vendors_list = vendors_df.select("id", "name").collect()
...
filtered_items = items_df.filter(
    ...
    (col("active") == 1)
).select("id", "name", "vendor_id", "times_purchased")
...
# print(f"Filtered vendors: {filtered_vendors.count():,}")

# Step 3: Create Item Type Market Basket Data
print("\n🛒 Step 3: Creating Vendors market basket transactions...")

# Join to get item type information and create transactions by item type
vendors_transactions = filtered_order_lines \
    .join(filtered_items.select(col("id").alias("item_id"), "vendor_id"), "item_id") \
    .join(vendors_df.select(col("id").alias("vendor_id"), col("name").alias("vendor_name")), "vendor_id") \
    .groupBy("order_id") \
    .agg(
        collect_set("vendor_name").alias("vendors"),
        sum("quantity").alias("total_quantity"),
        sum(col("quantity") * col("price_per_unit")).alias("total_value")
    ) \
    .filter(size(col("vendors")) >= 2)  # Only consider orders with 2+ different item types

total_transactions = vendors_transactions.count()
...
sample_display = vendors_transactions.limit(5).collect()
for i, row in enumerate(sample_display, 1):
    print(f"   Transaction {i}: {', '.join(row['vendors'])} (Qty: {row['total_quantity']}, Value: ${row['total_value']:.2f})")
...
print("PCY Pass 1: Counting vendors and hashing pairs...")

# Collect transactions to driver for PCY processing
print("Collecting sample transactions for PCY...")
sample_transactions = vendors_transactions.sample(False, np.min([1.0, 100000/total_transactions])).collect()
...
for row in sample_transactions:
    vendors = row['vendors']
    # Count individual item types
    for item_type in vendors:
        item_type_counts[item_type] += 1
    
    # Hash all pairs and count buckets
    for i in range(len(vendors)):
        for j in range(i+1, len(vendors)):
            bucket = hash_pair(vendors[i], vendors[j])
            bucket_counts[bucket] += 1

# Find frequent 1-itemsets (item types)
frequent_1_vendors = {item_type for item_type, count in item_type_counts.items() 
                        if count >= abs_min_support * (sample_size / total_transactions)}

print(f"Frequent 1-itemsets (vendors) found: {len(frequent_1_vendors)}")
print("Frequent item types:", list(frequent_1_vendors))
...
for row in sample_transactions:
    vendors = [item_type for item_type in row['vendors'] if item_type in frequent_1_vendors]
...
print(f"Frequent 2-itemsets (vendors pairs) found: {len(frequent_2_itemsets)}")
...
for row in sample_transactions:
    vendors = set(item_type for item_type in row['vendors'] if item_type in frequent_1_vendors)
    
    for candidate in candidate_3_itemsets:
        if set(candidate).issubset(vendors):
...
all_vendors = list(frequent_1_vendors)
n_types = len(all_vendors)

# Create co-occurrence matrix
cooccurrence_matrix = np.zeros((n_types, n_types))
type_to_idx = {item_type: i for i, item_type in enumerate(all_vendors)}

for row in sample_transactions:
    vendors = [it for it in row['vendors'] if it in frequent_1_vendors]
    for i in range(len(vendors)):
        for j in range(len(vendors)):
            if i != j:
                idx_i = type_to_idx[vendors[i]]
                idx_j = type_to_idx[vendors[j]]
...
frequent_1_data = prepare_itemset_data(frequent_1_vendors, item_type_counts, 1)
...
with open(f'{OUTPUT_DIR}/item_type_frequent_itemsets.txt', 'w') as f:
    ...
    for item_type in sorted(frequent_1_vendors):
...
with open(f'{OUTPUT_DIR}/item_type_association_rules.txt', 'w', encoding="utf-8") as f:
...
print(f"   • Frequent 1-itemsets (item types): {len(frequent_1_vendors)}")
...
if frequent_1_data:
    ...
    vendors = [item['itemset'] for item in top_types]
    supports = [item['support'] for item in top_types]
    
    bars = plt.barh(range(len(vendors)), supports, color='lightblue', alpha=0.8)
    plt.yticks(range(len(vendors)), vendors)
    plt.xlabel('Support')
...
counts = [len(frequent_1_vendors), len(frequent_2_itemsets), len(frequent_3_itemsets)]
...
values = [len(item_type_counts), len(frequent_1_vendors), len(pair_counts), 
...
if len(all_vendors) > 1:
    ...
    sns.heatmap(cooccurrence_matrix, 
                xticklabels=all_vendors, 
                yticklabels=all_vendors,
                annot=True, 
                fmt='.3f', 
                cmap='YlOrRd', 
                mask=mask,
...
plt.subplot(3, 4, 8)
transaction_sizes = [len(row['vendors']) for row in sample_transactions]
